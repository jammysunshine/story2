...
async function migrateGCSData() {
  ...
  try {
    ...
    const booksToUpdate = await db.collection<Book>('books').find({
      $or: [
...
async function copyBucket(sourceBucket: any, destBucket: any) {
  ...
  for (const file of files) {
    try {
      ...
      log.error(`Failed to copy ${file.name}: ${copyError}`);
    }
  }
