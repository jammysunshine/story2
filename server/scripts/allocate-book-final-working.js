...
async function transferBooks() {
  ...
  try {
    ...
    while (await cursor.hasNext()) {
      ...
      console.log(`\n📦 Processing book ${currentIndex}/${totalBooks} (ID: ${book._id.toString()})`);
...
const timeoutPromise = new Promise((_, reject) => {
  setTimeout(() => reject(new Error('Script timed out after 10 minutes')), 600000); // 10 minutes
});

Promise.race([transferBooks(), timeoutPromise])
  .catch(error => {
    if (error.message === 'Script timed out after 10 minutes') {
      console.log('⚠️ Script timed out after 10 minutes. This is expected for large datasets.');
    } else {
