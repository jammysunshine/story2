...
async function cleanupBinaryData() {
  ...
  try {
    ...
    console.log(`   Total database size: ${formatBytes(dbStats.dataSize as number)}`);
    console.log(`   Total storage size: ${formatBytes(dbStats.storageSize as number)}`);
    console.log(`   Average object size: ${formatBytes(dbStats.avgObjSize as number)}`);
...
function findBinaryFields(obj: any, prefix: string = '', results: BinaryField[] = []): BinaryField[] {
...
function estimateBinarySize(obj: any, path: string): number {
