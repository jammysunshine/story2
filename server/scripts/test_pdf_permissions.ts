...
async function testPdfPermissions() {
  ...
  try {
    ...
    try {
      ...
      console.error('❌ Failed to download test file content:', (downloadError as Error).message);
    }
    ...
    try {
      ...
      console.error('❌ Failed to create signed URL for PDF bucket:', (signError as Error).message);
    }
    ...
    try {
      ...
      console.error('⚠️ Warning: Failed to clean up test file:', (cleanupError as Error).message);
    }
    ...
    if (imagesBucketName) {
      ...
      try {
        ...
        console.error('❌ Failed to access Images bucket:', (imageError as Error).message);
      }
    } else {
    ...
    console.error('💥 Error testing GCS permissions:', (error as Error).message);
