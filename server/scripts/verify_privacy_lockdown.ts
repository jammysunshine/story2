...
async function verifyPrivacyLockdown() {
  ...
  try {
    ...
    const users = await db.collection<User>('users').find({}).limit(5).toArray();
    ...
    const books = await db.collection<Book>('books').find({}).limit(5).toArray();
    ...
    if (process.env.GCS_IMAGES_BUCKET_NAME && process.env.GCS_PDFS_BUCKET_NAME) {
      ...
      try {
        ...
        console.log(`   ⚠️  Could not check bucket privacy: ${(bucketError as Error).message}`);
      }
    } else {
    ...
    const booksWithUrls = await db.collection<Book>('books').find({
      $or: [
    ...
    const booksIndexes = await db.collection<Book>('books').indexes() as IndexInfo[];
    const usersIndexes = await db.collection<User>('users').indexes() as IndexInfo[];
