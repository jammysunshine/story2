...
async function migrateMongoUrls() {
  ...
  try {
    ...
    const books = await db.collection<Book>('books').find({
      $or: [
        { 'pages.imageUrl': { $regex: new RegExp(OLD_IMAGES_BUCKET) } },
        { 'pdfUrl': { $regex: new RegExp(OLD_PDFS_BUCKET) } }
      ]
    }).toArray();
    ...
    for (const collectionInfo of collections) {
      ...
      // Check if this collection has any documents
...
function findUrlFields(obj: any, oldImageBucket: string, oldPdfBucket: string, visited: Set<any> = new Set()) {
...
function updateNestedUrls(
  obj: any, 
  oldImageBucket: string, 
  newImageBucket: string, 
  oldPdfBucket: string, 
  newPdfBucket: string
) {
