...
async function testLatestStoryGen() {
  ...
  try {
    ...
    const recentBooks = await db.collection<Book>('books').find({})
    ...
    console.error('💥 Error testing latest story generation:', (error as Error).message);
