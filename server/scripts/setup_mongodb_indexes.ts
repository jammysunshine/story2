...
async function setupIndexes() {
  ...
  try {
    ...
    const collectionsWithIndexes: CollectionSpec[] = [
      {
        name: 'books',
        indexes: [
    ...
    for (const collectionSpec of collectionsWithIndexes) {
      ...
      for (const indexSpec of collectionSpec.indexes) {
        try {
          ...
          log.error(`❌ Error creating index ${JSON.stringify(indexSpec.key)} on ${collectionSpec.name}:`, (indexErr as Error).message);
        }
      }
    }
