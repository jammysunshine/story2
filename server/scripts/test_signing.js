  ...
  try {
    ...
    for (const test of expirationTests) {
      try {
        ...
        console.log(`  ${test.name}: ❌ Failed - ${expError.message}`);
      }
    }
    ...
    if (process.env.GCS_IMAGES_BUCKET_NAME) {
      ...
      try {
        ...
        console.error(`❌ Failed to work with images bucket: ${imageError.message}`);
      }
    }
    ...
    try {
      ...
      console.log(`  Content-Disposition header: ❌ Failed - ${dispError.message}`);
    }
    ...
    try {
      ...
      console.log(`  Content-Type override: ❌ Failed - ${typeError.message}`);
    }
    ...
    try {
      ...
      console.error(`\n⚠️  Warning: Failed to clean up test file: ${cleanupError.message}`);
    }
    ...
    try {
      ...
      console.log(`  PDF 7-day signing: ❌ Failed - ${pdfSignError.message}`);
    }
    ...
    try {
      ...
      console.log(`  Image 15-min signing: ❌ Failed - ${imageSignError.message}`);
    }
    ...
    console.error('💥 Error testing GCS signing:', error.message);
